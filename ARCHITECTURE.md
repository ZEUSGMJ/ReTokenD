...
Spotify applies a 6-month refresh-token lifetime to apps created on or after June 18, 2026 and to existing apps from July 20, 2026; refreshing does **not** extend that window. Instead of pasting a new refresh token into every consuming project twice a year, ReTokenD owns the refresh token(s) centrally: consumers ask it for short-lived access tokens, and when the window closes, one dashboard re-authorization flow heals every consumer at once.
...
```
proxy.ts                  Request interceptor (Next 16 "proxy" convention). Node runtime.
                          Fail-closed matcher: exact bearer APIs bypass the session check;
                          all other dynamic routes are gated; adds X-Robots-Tag.
app/
...
lib/
  ...
  proxy-paths.ts          Exact-match session-exempt paths for the proxy (/api/token, /api/check).
...
Next.js 16 runs `proxy.ts` on the Node.js runtime. It remains storage-free and performs only the optimistic cookie check:

- **`lib/session.ts`** — Web Crypto-only. Signs/verifies cookies with `crypto.subtle` (HMAC-SHA256). Imported by the proxy *and* by Node routes.
...
Do not import `lib/auth.ts`, `lib/crypto.ts`, or `lib/session-server.ts` into `lib/session.ts`; keep `proxy.ts` storage-free.

## Storage abstraction

Everything reads/writes through `storage` ([lib/storage/index.ts](lib/storage/index.ts)) — a small `StorageAdapter` interface (`get/set/setWithTTL/del/incr/acquireLock/releaseLock`). `incr(key, windowSeconds)` is an atomic counter (INCR, with the expiry window set only when the key is newly created) used by the login rate limiter. Locks store a unique owner and release with an atomic compare-and-delete. Two adapters:
...
| `refresh_lock` | Owner-tagged single-flight lock (`SET NX EX`) around the Spotify refresh call. |
...
5. **Single-flight refresh**: try an owner-tagged `refresh_lock` (`SET NX EX 10`). Losers poll the cache ~2s (4 × 500ms) and return the winner's token; if the winner stalls, a loser **re-reads `refresh_token` and `reauth_required`** (409 if now flagged) before falling through to a fallback refresh with the fresh token (safety valve). A holder releases only if its owner value still matches.
...
3. The code is exchanged for tokens using the profile's credentials. The response's `refresh_token` is validated present, then stored before `issued_at` is reset (**the only place this happens**). Only after both writes succeed does ReTokenD drop the cached access token and clear `reauth_required` and notification-dedupe flags.
...
**Auth has optimistic and authoritative checks.** The Node-runtime proxy stays storage-free and verifies only the cookie **signature + age**. Its exact `/api/token` and `/api/check` bypass handles their independent bearer auth; similarly prefixed routes remain session-gated. The **generation check** (server-side revocation) needs storage, so it runs at the top of `/` (`app/page.tsx`), `/api/login`, and `/api/callback` via `isSessionCurrent()` ([lib/session-server.ts](lib/session-server.ts)), redirecting to `/login` when the cookie's `gen` no longer matches Redis. `logout()` bumps `session:generation` (revoking every outstanding cookie) then deletes the cookie. Server actions are protected because they POST to `/`, which the proxy covers.
...
| Proxy | Runs in the Node server process | Node.js runtime |
